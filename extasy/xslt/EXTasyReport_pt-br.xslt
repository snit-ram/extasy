...
>
    ...
    <table border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        <td style="padding: 3px 3px 3px 3px" class="storiesHeader">
          Histórias [<a href="javascript:" onclick="toggleAll();">Colapsar/Expandir Todas</a>]
        </td>
      </tr>
      <tr>
        <tr>
  ...
  <xsl:template match="/report/stories/story">
    <div style="margin-top:6px;">
      ...
      <b>
        História <xsl:value-of select ="@index"/>: [<a href="javascript:">
          <xsl:attribute name="onclick">toggle('div_<xsl:value-of select = "@index" />');</xsl:attribute>Colapsar/Expandir
        </a>]
      </b>
      <br />
  ...
  <xsl:template match="/report/stories/story/scenario">
    <div style="padding-top:6px; width:100%;">
      <b>
        ...
        Cenário <xsl:value-of select ="@index"/>
      ...
      <tr>
        <td style="padding: 3px 3px 3px 3px">
          <xsl:if test="@failedScenarios>0">
            <span class="runFailed">Testes falharam!</span>
          </xsl:if>
          <xsl:if test="@failedScenarios=0">
            <span class="runSucceeded">Testes com sucesso!</span>
          </xsl:if>
        </td>
      </tr>
      <tr>
        <td class="summary" style="padding: 3px 3px 3px 3px">
          <b>Sumário:</b><br />
          Número de histórias: <xsl:value-of select ="@totalStories" /><br />
          Número de cenários: <xsl:value-of select ="@totalScenarios" /><br />
          Cenários com sucesso: <span class="successfulTests">
            <xsl:value-of select ="@successfulScenarios" /> (<xsl:value-of select ="@percentageSuccessful" />%)
          </span><br />
          Cenários com erro: <span class="failedTests">
  ...
  <xsl:template match="/report/footer">
    <tr>
      <td class="footer">
        EXTasy - Versão <xsl:value-of select ="@version"/> - <a href="http://www.extasy.org">http://www.extasy.org</a>
      </td>
    </tr>
  </xsl:template>
</xsl:stylesheet>
