...
from extasy.common import Settings, URLChecker
...
class PageRegistry(object):
    ...
    def resolve(cls, settings, url, must_raise=True, abspath_func=abspath, exists_func=exists):
        ...
        if not isinstance(settings, Settings):
            if must_raise:
                raise TypeError('PageRegistry.resolve takes a extasy.common.Settings object first parameter. Got %r.' % settings)
            else:
                return None
...
class Page(object):
    ...
    def quick_register(self, element_key, element_selector):
        if not lxml_available:
            raise RuntimeError("You can't use CSS selectors unless you install lxml. Installing it is pretty easy. Check our docs at http://www.extasy.org to know more.")
