...
import textwrap
import sys, optparse
from extasy.core import EXTasyCore
from extasy.common import Status
from extasy.story_runner import StoryRunner, ParallelStoryRunner
from extasy import Version, Release
from extasy.colored_terminal import ProgressBar, TerminalController

__version_string__ = "extasy %s (release '%s')" % (Version, Release)
...
def create_progress(verbosity):
    ...
    scenarios_ran = 0
    if not no_progress:
        prg = ProgressBar("EXTasy - %s" % __version_string__, verbosity)
...
def main(arguments=sys.argv[1:]):
    ...
    parser.add_option("-p", "--pattern", dest="pattern", default="*.story", help="File pattern. Defines which files will get executed [default: %default].")
    ...
    parser.add_option("-L", "--languagesdir", dest="languages_dir", default=None, help="Languages Directory. Defines where EXTasy will search for language dictionaries  [default: %default].")
    ...
    parser.add_option("-a", "--actionsdir", action="append", dest="actions_dir", default=[], help="Actions directory. Defines where the EXTasy actions are. Chances are you don't need to change this parameter [default: %default].")
    parser.add_option("-A", "--customactionsdir", action="append", dest="custom_actions_dir", default=[], help="Custom Actions directory. Defines where the EXTasy custom actions are. If you don't change this parameter EXTasy will use the tests directory [default: %default].")
    parser.add_option("-P", "--pagesdir", action="append", dest="pages_dir", default=[], help="Pages directory. Defines where the EXTasy custom pages are. If you don't change this parameter EXTasy will use the tests directory [default: %default].")
    parser.add_option("-H", "--hooksdir", action="append", dest="hooks_dir", default=[], help="Hooks directory. Defines where EXTasy hooks are. If you don't change this parameter EXTasy will use the tests directory [default: %default].")
    ...
    #throws
    parser.add_option("-T", "--throws", dest="should_throw", default=False, help="Should Throw. Defines whether EXTasy console should throw an exception when tests fail. This is useful to set to True if you are running EXTasy inside unit tests [default: %default].")

    #reporter
    parser.add_option("-R", "--report", dest="write_report", default="true", help="Should write report. Defines if EXTasy should write an html report after each run [default: %default].")
    ...
    parser.add_option("--suppresswarnings", action="store_true", dest="suppress_warnings", default=False, help="Suppress EXTasy warnings [default: %default].")
    ...
    result = oEXTasy.run_tests(actions_dir=options.actions_dir,
