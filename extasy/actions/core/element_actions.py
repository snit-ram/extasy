...
u'''
Element actions can be used for any registered element (more about registering elements at "[[Creating custom Pages]]" section). The majority of EXTasy's actions are in this category, like clicking elements or verifying that they contain a given style.
...
This action is really useful when you have some processing done (maybe AJAX) before an element is dynamically removed or hidden.
    ...
    def execute(self, context, element_type, element_name, timeout):
        ...
        if not timeout:
            timeout = extasy.DEFAULT_WAIT_FOR_DISAPPEAR_TIMEOUT
        timeout = int(timeout)
